No one need to remember the IP addresses of websites due to the Domain Name System (DNS), which is responsible for properly mapping a domain name to an IP address.
...
There are many types of DNS records; However we will focus on the four below
- `A record`: The A (Address) record maps a hostname to one or more IPv4 addresses. For example, you can set `example.com` to resolve to `172.17.2.172`
- `AAAA record`: The AAAA record is similar to A record, but it is for ipv6. Remember that it is AAAA (quad-A), as AA and AAA would refer to a battery size; further more, AAA refers to Authentication, Authorization, and accounting; neither falls under DNS.
...
If you want to lookup the IP address of a domain from the command line, you can use a tool called `nslookup`
```shell-session
